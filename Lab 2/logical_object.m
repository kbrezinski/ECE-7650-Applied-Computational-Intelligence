...
%             fontSize = 20;
            ...
            plot(obj.feature(pos,1),obj.feature(pos,2),'r+'); hold on
            plot(obj.feature(neg,1),obj.feature(neg,2),'bo')
            line([0,100],[-theta(1)/theta(3),(-(100*theta(2)+theta(1))/theta(3))])
            xlim([min(obj.feature(:,1)),max(obj.feature(:,1))]);ylim([min(obj.feature(:,2)),max(obj.feature(:,2))])
            xlabel('Exam 1 Score'); ylabel('Exam 2 Score'); legend('Admitted','Not Admitted','Decision Boundary','location','best');
